...
func (userApi *UserApi) Register(c *gin.Context) {
	...
	//获得会话实例，用于操作会话数据，本项目中存储的会话数据包括邮箱验证码，邮箱和过期时间。
...
// TokenNext 处理用户登录后的令牌生成和管理
// 参数：
//   - c: Gin 上下文，用于处理请求和响应
//   - user: 登录成功的用户信息
// 功能：
//   1. 检查用户是否被冻结
//   2. 生成访问令牌（Access Token）和刷新令牌（Refresh Token）
//   3. 处理多地点登录拦截逻辑
//   4. 管理 Redis 中的 JWT 存储
//   5. 设置刷新令牌到 Cookie 并返回登录响应
func (userApi *UserApi) TokenNext(c *gin.Context, user database.User) {
	...
	// 创建基础 Claims，包含用户的基本信息
	baseClaims := request.BaseClaims{
		UserID: user.ID,     // 用户 ID
		UUID:   user.UUID,   // 用户 UUID
		RoleID: user.RoleID, // 用户角色 ID
	}
	...
	if !global.Config.System.UseMultipoint {
		// 未开启多地点登录拦截，直接设置刷新令牌并返回
		// 计算刷新令牌的有效期（秒）
		expiry := int(refreshClaims.ExpiresAt.Unix() - time.Now().Unix())
		utils.SetRefreshToken(c, refreshToken, expiry)
		// 将用户 ID 存储到上下文中
		c.Set("user_id", user.ID)
		// 返回登录成功响应，包含用户信息和访问令牌
		response.OkWithDetailed(response.Login{
			User:                 user,
			AccessToken:          accessToken,
			AccessTokenExpiresAt: accessClaims.ExpiresAt.Unix() * 1000, // 转换为毫秒
		}, "Successful login", c)
		return
	}

	// 开启了多地点登录拦截，检查 Redis 中是否已存在该用户的 JWT
	if jwtStr, err := jwtService.GetRedisJWT(user.UUID); errors.Is(err, redis.Nil) {
		// Redis 中不存在该用户的 JWT，设置新的刷新令牌
		...
		// Redis 中已存在该用户的 JWT，需要将旧的 JWT 加入黑名单
		var blacklist database.JwtBlacklist
		blacklist.Jwt = jwtStr // 旧的刷新令牌
