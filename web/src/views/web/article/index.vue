            ...
            <div class="operation">
              <div class="comment-tool">
                <el-popover width="502" trigger="click">
                  <template #reference>
                    <el-avatar src="/emoji/emoji_01.jpg"/>
                  </template>
                  <template #default>
                    <el-image
                        v-for="number in numbers"
                        :src="'/emoji/emoji_'+number+'.jpg'"
                        @click="content=content+'![]('+'/emoji/emoji_'+number+'.jpg'+')'"
                    />
                  </template>
                </el-popover>
              </div>
...
const getArticleInfo = async () => {
  ...
  if (!articleID.value) {
    console.error('文章ID不存在，路由参数:', route.params)
    await router.push({name: "404"})
    return
  }
  
  try {
    console.log('调用文章详情API，ID:', articleID.value)
    const res = await articleInfoByID(articleID.value as string)
    if (res.code === 0) {
      articleInfo.value = res.data
    } else {
      console.error('获取文章信息失败:', res.msg)
      await router.push({name: "404"})
    }
  } catch (error) {
    console.error('调用文章详情API失败:', error)
    await router.push({name: "404"})
  }
}
...
const numbers = ref(Array.from({length: 33}, (_, i) => (i + 1).toString().padStart(2, '0')));
